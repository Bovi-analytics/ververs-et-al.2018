...
- Days since arrival at the farm at dehorning (proxy for age)
- Quarter of dehorning (proxy for season)
- Days since calving at dehorning
- Calculated number of calvings
- Quarter at calving
...
```{r, echo=FALSE}
summary(baseline <- glm(G_TOTAL~ 1, df.cv.all, family = gaussian(link="log")))
summary(glm2 <- glm(G_TOTAL~ N_DEHORNINGS, df.cv.all, family = gaussian(link="log")))
summary(glm3 <- glm(G_TOTAL~ CALC_AGE, df.cv.all, family = gaussian(link="log")))
summary(glm4 <- glm(G_TOTAL~ DH_QUARTER, df.cv.all, family = gaussian(link="log")))
summary(glm5 <- glm(G_TOTAL~ B_QUARTER, df.cv.all, family = gaussian(link="log")))
summary(glm6 <- glm(G_TOTAL~ GENDER, df.cv.all, family = gaussian(link="log")))
```
...
```{r, echo=FALSE}
summary(baseline <- glm(G_AH_W~ 1, df.cv.all.f, family = gaussian(link="log")))
summary(glm2 <- glm(G_AH_W~ N_DEHORNINGS, df.cv.all.f, family = gaussian(link="log")))
summary(glm3 <- glm(G_AH_W~ CALC_AGE, df.cv.all.f, family = gaussian(link="log")))
summary(glm4 <- glm(G_AH_W~ DH_QUARTER, df.cv.all.f, family = gaussian(link="log")))
summary(glm5 <- glm(G_AH_W~ B_QUARTER, df.cv.all.f, family = gaussian(link="log")))
summary(glm6 <- glm(G_AH_W~ GENDER, df.cv.all.f, family = gaussian(link="log")))
```
